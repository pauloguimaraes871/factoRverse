# factoRverse ![factoRverse logo](inst/images/factoRverse.png)

An end-to-end toolkit for **factor investing** research and deployment:
engineer point-in-time signals, tame the factor zoo with disciplined
multiple-testing inference, blend signals with machine learning, and
build cost- and constraint-aware portfolios, all through one consistent,
auditable interface.
...
- **Configuration is modular and swappable.** Each workflow is driven by
  ...
  `alpha_test_strategy`, which may carry `bayesian_model_parameters`).
  Pieces snap together and can be reused across runs.
- **Runs are self-identifying.** Named configs propagate into the
  `backtest_identifier` of every `*_results` object, so experiments stay
  labelled and comparable.

Two naming conventions recur throughout: objects suffixed `_m_df` are
`meta_dataframe`s (long panels) and `_m_xts` are `meta_xts` (wide time
series); and every workflow follows the same shape: **`config` + data
objects, then `run_*()`, then a `*_results` object.**
...
``` mermaid
flowchart TB
    subgraph S0 ["0. Data layer: point-in-time preprocessing"]
        F0["compute_* signal engineering (self-contained)
        map_recipe_timewise: any recipes pipeline, one date at a time
        tickers_catalog, target_m_df
        uses: recipes, dplyr, purrr, xts/zoo"]
    end

    subgraph P1 ["1. Characteristic portfolios: run_port_backtest"]
        F1["EW, SW, CW, CS, HRP, MMAF built in-house
        RP via riskParityPortfolio; MVO via PortfolioAnalytics
        liquidity/turnover/concentration rules, costs, trade log
        uses: PortfolioAnalytics, riskParityPortfolio, PerformanceAnalytics"]
    end

    subgraph P2 ["2. Signal selection: run_ss_backtest"]
        F2["FWER (Bonferroni, Holm) and FDR (BH, BY) control
        hierarchical CAPM: no-pooled or partial-pooled
        Bayesian shrinkage, user or data-informed priors
        uses: lme4, lmerTest, brms/Stan, tidybayes"]
    end

    subgraph P3 ["3. Signal blending: run_sb_backtest"]
        F3["ML: glmnet, ranger, xgboost, keras/TensorFlow
        tuning: grid, random, Bayesian (ParBayesianOptimization)
        heuristic blenders enable factor timing; meta-ensembles
        feature importance and explain_prediction"]
    end

    subgraph P4 ["4. Deployment: run_port_backtest(sb_results)"]
        F4["blended score drives the final book
        full constraint machinery, costs, audit trail"]
    end

    SPW(["set_portfolio_weights()
    one weighting engine"])
    CIU(["classify_investment_universe()
    one eligibility grammar"])
    SPF(["summarize_performance()
    one CAPM metrics engine"])
...
(signals screened by statistical significance) share one eligibility
engine. Hyperparameter tuning and Bayesian MCMC parallelize through the
`future` backend.

Every `*_results` object also answers
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) with an
interactive menu: 76 built-in plots across the five results classes (18
for portfolio backtests, 16 for cohorts, up to 16 for signal selection
including posterior diagnostics, up to 18 for signal blending including
the feature-importance suite, and 8 for meta-ensembles), all in a
consistent palette.
...
Two workflows rely on external toolchains that are not installed
automatically with the package:

- **Neural-network signal blending** needs Python with TensorFlow and
  Keras, configured through the `tensorflow`/`keras` packages. After
  installing `factoRverse`, run
  [`keras::install_keras()`](https://rdrr.io/pkg/keras/man/install_keras.html)
  once to set up a compatible Python environment.
- **Hierarchical Bayesian signal selection** needs a working C++
  toolchain and Stan, provided through `brms` (on Windows, install
  Rtools; see the `brms` and `rstan` installation guides).
...
The snippets below sketch the public API end to end. They are
illustrative (the data objects are assumed to already exist); see the
function reference on the [package
website](https://pauloguimaraes871.github.io/factoRverse/) or the
individual help pages
(e.g. [`?run_port_backtest`](https://pauloguimaraes871.github.io/factoRverse/reference/run_port_backtest.md))
for argument-level detail.

### 0. Feature engineering & point-in-time preprocessing

Wrap a panel in a validated, self-documenting container, engineer
signals from the factor-zoo literature, and preprocess **one date at a
time** so no future information leaks into the past.
...
A `tickers_catalog` (stable, rename-proof `perm_id`s and
listing/delisting bookkeeping) and
[`create_target_m_df()`](https://pauloguimaraes871.github.io/factoRverse/reference/create_target_m_df.md)
(dozens of forward, risk-adjusted targets) round out the data layer.

### 1. Characteristic portfolios

Rank stocks on a signal, invest in the top quantile, and backtest the
resulting long-only portfolio with realistic liquidity filters and
transaction costs.
...
Which signals are *real*? Pass the backtested returns of your candidate
portfolios through disciplined, walk-forward inference on CAPM alpha,
with frequentist multiple-testing control (FWER: Bonferroni/Holm; FDR:
BH/BY) or Bayesian hierarchical shrinkage that partially pools alphas
within economically motivated **themes**. This is the workflow studied
in *The Factor Zoo Revisited* (see below).
...
Priors can be user-defined or estimated from an exogenous global
dataset.
...
and predictions are interpretable end to end via global-surrogate
...
framework. Choose a backend with
...
the *Factor Zoo Revisited* paper. Run `citation("factoRverse")` for the
current entry.
