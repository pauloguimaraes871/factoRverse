...
test_that("OLS - run_sb_backtest works with toy_preprocessed_features_and_targets + signal_selection", {
  ...
  ), order.by = unique(toy_preprocessed_features$dates)), type = "returns")
...
test_that("Signal Weights + Custom Signal Universe Metrics - run_sb_backtest works with toy_preprocessed_features_and_targets", {
  ...
  tickers <- colnames(toy_preprocessed_features)[-c(1:3)] %>% as.character()
  dates <- c("2022-09-15", "2022-11-15", "2022-02-15", "2023-04-15", "2023-06-15") %>% as.Date()
  suppressWarnings(custom_signal_universe_metrics_m_df <- expand.grid(tickers, dates, stringsAsFactors = FALSE) %>% dplyr::rename(tickers = Var1, dates = Var2) %>%
    dplyr::mutate(id = paste0(tickers,"-",dates)) %>%
  ...
  #1st rebalancing
  first_most_recent_derived_signal_universe_m_d_ref <- derived_signal_universe_m_df %>% dplyr::filter(dates == "2022-11-15") %>% dplyr::mutate(is_eligible = 1)
  ...
  first_sb_port <- fit_sb_model(sb_algorithm = "sw", target_fwd_name = "fwd_premium_3m",
                                ...
                                keras_architecture_parameters = NULL, optimal_hyper = NULL, chosen_eval_metric_translated = NULL,
                                selected_cov_matrix_benchmark_m_xts_upd_ref = selected_cov_matrix_benchmark_m_xts_upd_ref,
                                most_recent_custom_signal_weights_m_d_ref = NULL, verbose = TRUE,
...
test_that("EW - run_sb_backtest works with toy_preprocessed_features_and_targets", {
  ...
  ##EW Config
  ew_config <- create_sb_backtest_config(sb_algorithm = "ew", rebalancing_months = 7, training_sample_size = 5, target_fwd_name = "fwd_premium_3m",
                                         chosen_signals_and_positions = chosen_signals_and_positions)
  ...
  #1st rebalancing
  most_recent_signal_universe_m_d_ref <- data.frame(tickers = colnames(toy_preprocessed_features)[-c(1:3)], dates = as.Date("2022-11-15")) %>%
    dplyr::mutate(tickers = ifelse(tickers == "idio_vol_mrkt_ewma", "low_idio_vol_mrkt_ewma", tickers)) %>%
    dplyr::mutate(id = paste0(tickers,"-", dates), .before = tickers) %>% dplyr::mutate(is_eligible = as.integer(tickers %in% c(
      names(chosen_signals_and_positions), "low_idio_vol_mrkt_ewma")))
  ...
  expect_equal(sb_backtest_results@final_sb_model@model@weights, rep(0.166666667, 6))
...
test_that("RP - run_sb_backtest works in full integration with run_port_backtest + run_ss_backtest with toy_preprocessed_features_and_targets + signal_selection", {
  ...
  suppressMessages(suppressWarnings({
    sb_backtest_results <- run_sb_backtest(
      ...
      target_m_df = create_meta_dataframe(target_m_df),
      gsm_algorithm = "tree",
  ...
  #1st rebalancing
  #Most recent xts
  most_recent_signal_universe_m_d_ref <- ss_results@signal_universe_m_df@data %>% dplyr::filter(dates == "2023-02-15")
  selected_backtest_returns_m_xts_upd_ref <- backtest_returns_m_xts@data["2022-12-15/2023-02-15",
                                                                        most_recent_signal_universe_m_d_ref %>% dplyr::filter(is_eligible == 1) %>% dplyr::pull(tickers)]
  selected_cov_matrix_benchmark_m_xts_upd_ref <- benchmark_returns_m_xts@data["2022-12-15/2023-02-15", "ibov"]

  #Features Objects
  selected_signals_m_df <- signals_m_df@data %>% dplyr::mutate(low_vol_36m = vol_36m*-1) %>% dplyr::select(-vol_36m) %>%
    dplyr::select(id, tickers, dates, most_recent_signal_universe_m_d_ref %>% dplyr::filter(is_eligible == 1) %>% dplyr::pull(tickers))

  selected_features_first_rebal <- selected_signals_m_df[which(selected_signals_m_df$dates %in% c("2022-10-15", "2022-11-15")),]

  #Targets
  targets_first_rebal_m_df <- target_m_df[which(target_m_df$dates %in% c("2022-10-15", "2022-11-15")),]
  ...
  first_sb_port <- fit_sb_model(sb_algorithm = "rp", target_fwd_name = "fwd_premium_3m",
                                ...
                                cov_matrix_sample_size = 2, cov_estimation_method = "ewma", active_returns = TRUE, groups_m_d_ref = NULL,
                                custom_objective_translated = NULL, huber_delta = 1, quantile_tau = 0.5, early_stop = NULL,
                                keras_architecture_parameters = NULL, optimal_hyper = NULL, chosen_eval_metric_translated = NULL,
                                selected_cov_matrix_benchmark_m_xts_upd_ref = selected_cov_matrix_benchmark_m_xts_upd_ref,
                                rp_method = "cyclical-spinu"
  ...
  for(i in 1:length(dates_first_prediction)){
    ...
    target_first_prediction <-  target_m_df$fwd_premium_3m[which(target_m_df$dates %in% dates_first_prediction[i])]
  ...
  selected_features_second_rebal <- selected_signals_m_df[which(selected_signals_m_df$dates %in% c("2022-10-15", "2022-11-15", "2022-12-15",
  ...
  #Predict
  dates_second_prediction <- c("2023-04-15")
  #Resulting obj
  prediction_list_second_prediction <- list()
  y_list_second_prediction <- list()
  error_list_second_prediction <- list()
  ...
  for(i in 1:length(dates_second_prediction)){
    ...
    target_second_prediction <-  target_m_df$fwd_premium_3m[which(target_m_df$dates %in% dates_second_prediction[i])]
  ...
  #Start object
  feature_importance_m_df1 <- ss_results@signal_universe_m_df@data %>% dplyr::filter(dates == "2023-02-15") %>%
    dplyr::select(tickers, theme_ss_bench_weights, theme_sb_bench_weights, theme, is_eligible)
  #Get coefs
  tree_coefs <- data.frame(tickers = names(gsm$variable.importance), importance = gsm$variable.importance, row.names = NULL)
  ...
  #Join and adjust
  feature_importance_m_df1 <- feature_importance_m_df1 %>% dplyr::full_join(tree_coefs, by = "tickers")
  feature_importance_m_df1$dates <- as.Date("2023-02-15") #Rebalance date
  feature_importance_m_df1$id <- paste0(feature_importance_m_df1$tickers, "-", feature_importance_m_df1$dates)
  #Reorder
  feature_importance_m_df1 <- feature_importance_m_df1 %>% dplyr::select(id, tickers, dates,
                                                                         importance, normalized_importance, theme, theme_ss_bench_weights, theme_sb_bench_weights, is_eligible)
  ...
  #Start object
  feature_importance_m_df2 <- ss_results@signal_universe_m_df@data %>% dplyr::filter(dates == "2023-04-15") %>%
    dplyr::select(tickers, theme_ss_bench_weights, theme_sb_bench_weights, theme, is_eligible)
  #Get coefs
  tree_coefs <- data.frame(tickers = names(gsm$variable.importance), importance = gsm$variable.importance, row.names = NULL)
  ...
  #Join and adjust
  feature_importance_m_df2 <- feature_importance_m_df2 %>% dplyr::full_join(tree_coefs, by = "tickers")
  feature_importance_m_df2$dates <- as.Date("2023-04-15") #Rebalance date
  feature_importance_m_df2$id <- paste0(feature_importance_m_df2$tickers, "-", feature_importance_m_df2$dates)
  #Reorder
  feature_importance_m_df2 <- feature_importance_m_df2 %>% dplyr::select(id, tickers, dates,
                                                                         importance, normalized_importance, theme, theme_ss_bench_weights, theme_sb_bench_weights, is_eligible)
...
test_that("GLMNET - run_sb_backtest works with no rebalancing, 1m target, grid_search as tuning method and rss as chosen eval metric",{
 ...
 #SB Config
  ...
  #Final Model
  expect_true(all.equal(coef(sb_backtest_results@final_sb_model@model), coef(glm.mod.refit)))
...
#Define your test
test_that("GLMNET - run_sb_backtest works with NAs in last target_fwd periods of target_m_df - toy_preprocessed_features_and_targets",{
  ...
  expected_results$outputs[[2]] <- oos_testing_eval_metrics
  expect_equal(expected_results$outputs[[2]][1,] %>% as.numeric(),sb_backtest_results@oos_testing_eval_metrics_m_xts@data %>% as.numeric())

  #Eval metrics
  non_nas <- which(!is.na(unlist(y_list)))
  consolidated_eval_metrics <- calculate_eval_metrics(pred = unlist(prediction_list)[non_nas],
                                                      target = unlist(y_list)[non_nas])[-1]
  ...
  #Final Model
  expected_results$outputs[[4]] <- glm.mod.refit
  expect_true(all.equal(coef(sb_backtest_results@final_sb_model@model), coef(glm.mod.refit)))


  #Validation lossess for chosen metric
  names(chosen_eval_metric_val) <-  c("2023-04-15", "2023-06-15")
  expected_results$outputs[[5]] <- chosen_eval_metric_val
  expect_equal(expected_results$outputs[[5]], sb_backtest_results@chosen_eval_metric_validation)

  #Best Hyoer
  expected_results$outputs[[6]] <- xts::xts(data.frame(
  ...
  expect_equal(expected_results$outputs[[6]], sb_backtest_results@best_hyperparameters_m_xts@data)


  #Validation loss metrics for hyper choice
  expected_results$outputs[[7]] <- xts::xts(validation_eval_hyper_choice, order.by = as.Date(c("2023-04-15", "2023-06-15")))
  expect_equal(expected_results$outputs[[7]], sb_backtest_results@validation_eval_metrics_hyper_choice_m_xts@data)
...
#Update sb backtest
test_that("update_sb_backtest works for GLMNET and 3m horizon (no testing sample), with new month an empty month", {
...
test_that("update_sb_backtest works for RF and 3m horizon, with new month an empty month", {
  ...
  #SB Backtest
  sb_config <- create_sb_backtest_config(sb_algorithm = "rf", target_fwd_name = "fwd_premium_3m",
                                         training_sample_size = 4, rebalancing_months = 11) %>%
    add_tuning_strategy(tuning_method = "grid_search", validation_sample_size = 3) %>%
  ...
  expect_equal(updated_sb_results@oos_sb_outputs_m_df@data, expected_results@oos_sb_outputs_m_df@data)
  expect_equal(updated_sb_results@oos_sb_outputs_m_df@current_date, expected_results@oos_sb_outputs_m_df@current_date)
  expect_equal(as.data.frame(updated_sb_results@oos_testing_eval_metrics_m_xts@data), as.data.frame(expected_results@oos_testing_eval_metrics_m_xts@data))
  expect_equal(updated_sb_results@chosen_eval_metric_validation, expected_results@chosen_eval_metric_validation)
  expect_equal(updated_sb_results@final_sb_model@model, expected_results@final_sb_model@model)
  expect_equal(coef(updated_sb_results@final_gsm), coef(expected_results@final_gsm))
  expect_equal(updated_sb_results@best_hyperparameters_m_xts@data, expected_results@best_hyperparameters_m_xts@data)
  expect_equal(updated_sb_results@validation_eval_metrics_hyper_choice_m_xts@data, expected_results@validation_eval_metrics_hyper_choice_m_xts@data)
  expect_equal(updated_sb_results@feature_importance_m_df@data, expected_results@feature_importance_m_df@data)
...
test_that("update_sb_backtest works for RP and 1m horizon, with new month an empty month", {
  ...
  #SB Backtest
  sb_config <- create_sb_backtest_config(sb_algorithm = "rp", target_fwd_name = "fwd_premium_1m",
                                         training_sample_size = 4, rebalancing_months = 11) %>%
    add_cov_est_method(cov_estimation_method = "ewma", cov_matrix_sample_size = 2, active_returns = TRUE, cov_matrix_benchmark = "ibov") %>%
    add_rp_parameters(rp_method = "cyclical-spinu")
  ...
  expect_warning(
    sb_results <- run_sb_backtest(config = sb_config, features_m_df = features_m_df, target_m_df = target_m_df,
                                  ...
                                  signal_themes_m_df = signal_themes_m_df, ss_backtest_results = ss_results, .test_seed = 123),
    "Normalization not found in workflow. It is advisable that data is normalized before being fed to run_sb_backtest."
  )
  ...
  #The problem is here, do not recreate fwd_return_m_df from scratch
  fwd_return_m_df <- create_meta_dataframe(
    fwd_return_m_df@data %>% dplyr::filter(!dates == as.Date("2023-04-15")) %>% dplyr::bind_rows(
    target_m_df@data %>% dplyr::select(id, tickers, dates, fwd_return_1m) %>%
      dplyr::filter(dates %in% as.Date(c("2023-04-15", "2023-05-15"))) %>%
      dplyr::mutate(fwd_return_1m = fwd_return_1m * rnorm(nrow(.), 10, 1))
    ) %>% dplyr::arrange(id),
    meta_dataframe_name = "fwd_return", type = "target"
  )
  ...
  #Just append a new obs to becn return instead of creating new
  benchmark_returns_m_xts <- create_meta_xts(
      rbind(benchmark_returns_m_xts@data, xts::xts(-1.05, order.by = as.Date("2023-05-15"))),
      type = "returns", asset_type = "benchmarks"
  )
  ...
  expect_warning(
    expected_results <-
      run_sb_backtest(
        ...
        features_m_df = features_m_df,
        ss_backtest_results = new_ss_backtest,
        port_backtest_cohort = new_port_backtest_cohort,
        benchmark_returns_m_xts = benchmark_returns_m_xts,
        signal_themes_m_df = signal_themes_m_df,
        config = sb_config,
        verbose = FALSE,
  ...
  expect_equal(as.data.frame(updated_sb_results@oos_testing_eval_metrics_m_xts@data), as.data.frame(expected_results@oos_testing_eval_metrics_m_xts@data))
  expect_equal(updated_sb_results@chosen_eval_metric_validation, expected_results@chosen_eval_metric_validation)
  expect_equal(updated_sb_results@final_sb_model@model@universe_m_d_ref@data, expected_results@final_sb_model@model@universe_m_d_ref@data)
  expect_equal(coef(updated_sb_results@final_gsm), coef(expected_results@final_gsm))
  expect_equal(updated_sb_results@feature_importance_m_df@data, expected_results@feature_importance_m_df@data)
  expect_equal(updated_sb_results@final_feature_importance_m_d_ref@data, expected_results@final_feature_importance_m_d_ref@data)
...
test_that("update_sb_backtest works for Custom Weights and 3m horizon, with 2 subsequent updates (empty month + rebalancing)", {
  ...
  frequentist_ss_config <- create_ss_backtest_config(initial_sample_size = 3, rebalancing_months = 5,
                                                     split_method = "expanding", config_name = "frequentist_ss", active_returns = TRUE,
                                                     chosen_signals_and_positions = chosen_signals_and_positions
  ) %>%
    add_alpha_test_strategy(model_structure = "no_pooled",
                            signal_significance_threshold = 1, p_correction_method = "none",
                            market_factor_proxy = "ibov", enable_theme_representativeness = TRUE)
  ...
  ss_results <-
    run_ss_backtest(frequentist_ss_config,
                    ...
                    signal_themes_m_df = signal_themes_m_df,
                    verbose = TRUE),
  "The following active return columns only contains positive values, compromising some calculations: book_yield, eps_yield, roe_3m, sharpe_6m, low_idio_vol_mrkt_ewma")

  #SB Backtest
  sb_config <- create_sb_backtest_config(sb_algorithm = "custom_weights", target_fwd_name = "fwd_premium_3m",
                                         training_sample_size = 4, rebalancing_months = 5)
  ...
  theme_ss_weights_m_df <- ss_results@signal_universe_m_df@data %>% dplyr::select(id, tickers, dates, theme_ss_bench_weights) %>%
    ...
    expect_warning(
      sb_results <- run_sb_backtest(config = sb_config, features_m_df = features_m_df, target_m_df = target_m_df,
                                    gsm_algorithm = "tree", custom_signal_weights_m_df = theme_ss_weights_m_df,
                                    ss_backtest_results = ss_results, .test_seed = 123),
      "Normalization not found in workflow. It is advisable that data is normalized before being fed to run_sb_backtest."
    )
  ...
  theme_ss_weights_m_df <- updated_ss_results@signal_universe_m_df@data %>% dplyr::select(id, tickers, dates, theme_ss_bench_weights) %>%
    dplyr::rename(weights = theme_ss_bench_weights) %>%
    create_meta_dataframe(meta_dataframe_name = "theme_ss_weights_123", type = "weights")
  ...
  theme_ss_weights_m_df <- updated_ss_results_2@signal_universe_m_df@data %>% dplyr::select(id, tickers, dates, theme_ss_bench_weights) %>%
    dplyr::rename(weights = theme_ss_bench_weights) %>%
    create_meta_dataframe(meta_dataframe_name = "theme_ss_weights_123", type = "weights")
  ...
  theme_ss_weights_m_df <- new_ss_backtest@signal_universe_m_df@data %>% dplyr::select(id, tickers, dates, theme_ss_bench_weights) %>%
    dplyr::rename(weights = theme_ss_bench_weights) %>%
    create_meta_dataframe(meta_dataframe_name = "theme_ss_weights_123", type = "weights")
  ...
  expect_equal(as.data.frame(updated_sb_results_2@oos_testing_eval_metrics_m_xts@data), as.data.frame(expected_results@oos_testing_eval_metrics_m_xts@data))
  expect_equal(updated_sb_results_2@chosen_eval_metric_validation, expected_results@chosen_eval_metric_validation)
  expect_equal(updated_sb_results_2@final_sb_model@model@universe_m_d_ref@data, expected_results@final_sb_model@model@universe_m_d_ref@data)
  expect_equal(coef(updated_sb_results_2@final_gsm), coef(expected_results@final_gsm))
  expect_equal(updated_sb_results_2@feature_importance_m_df@data, expected_results@feature_importance_m_df@data)
  expect_equal(updated_sb_results_2@final_feature_importance_m_d_ref@data, expected_results@final_feature_importance_m_d_ref@data)
...
###Metabacktesting
test_that("GLMNET - Metabacktesting works for glmnet in meta learning (rf + glmnet with ss are base_sb_results) - features_passthrough != none - meta_ss_config without ss_object", {
  ...
  oos_predictions_m_df <- consolidate_oos_sb_outputs_m_df(
    base_sb_backtest_outputs_list = base_sb_backtest_results_list,
  ...
  expect_equal(features_passthrough_and_positions, rf_results@sb_backtest_workflow[[length(rf_results@sb_backtest_workflow)]]$chosen_signals_and_positions)
  expect_equal(features_passthrough_and_positions, glmnet_results@sb_backtest_workflow[[length(glmnet_results@sb_backtest_workflow)]]$chosen_signals_and_positions)
  ...
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@sb_backtest_workflow$`2023-07-15`$chosen_signals_and_positions,adapted_chosen_signals_and_positions)
  ...
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$cons_oos[c(1:9)], rf_results@consolidated_eval_metrics$cons_oos)
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$cons_oos[c(10:18)], glmnet_results@consolidated_eval_metrics$cons_oos)
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$cons_oos[c(19:27)], meta_results@consolidated_eval_metrics$cons_oos)
  expect_equal(unique(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$testing_dates_range),
               c(paste0(min(rf_results@sb_backtest_workflow[[length(rf_results@sb_backtest_workflow)]]$dates_testing_sample),"-",
                        max(rf_results@sb_backtest_workflow[[length(rf_results@sb_backtest_workflow)]]$dates_testing_sample)),
               paste0(min(meta_results@sb_backtest_workflow[[length(meta_results@sb_backtest_workflow)]]$dates_testing_sample),"-",
                      max(meta_results@sb_backtest_workflow[[length(meta_results@sb_backtest_workflow)]]$dates_testing_sample))
               ))
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[1, -c(1:2)] %>% tibble::remove_rownames(),
  ...
  for (j in 1:length(sb_metabacktest_results@time_series_oos_testing_metrics)){
    ...
    expect_equal(na.omit(sb_metabacktest_results@time_series_oos_testing_metrics[[j]]@data[,3]) %>% as.numeric(), meta_results@oos_testing_eval_metrics_m_xts@data[,j] %>% as.numeric())
  }
  ...
  for (j in 1:length(sb_metabacktest_results@time_series_validation_metrics)){
    ...
    expect_equal(sb_metabacktest_results@time_series_validation_metrics[[j]]@data[,2] %>% na.omit() %>% as.numeric(),
                 glmnet_results@validation_eval_metrics_hyper_choice_m_xts@data[,j] %>% as.numeric())

    expect_equal(sb_metabacktest_results@time_series_validation_metrics[[j]]@data[,3] %>% na.omit() %>% as.numeric(),
...
test_that("EW - Metabacktesting works for EW in meta learning (rf + glmnet with ss are base_sb_results) - features_passthrough != none - meta_ss_config without ss_object", {
  ...
  expect_equal(sb_metabacktest_results@base_learners_oos_predictions_m_df@data, oos_predictions_m_df@data)
  #Check if chosen_signals_and_positions flow is correct
  expect_true(all(features_passthrough_and_positions %in% chosen_signals_and_positions))
  expect_true(all(features_passthrough_and_positions %in% rf_results@sb_backtest_workflow[[length(rf_results@sb_backtest_workflow)]]$chosen_signals_and_positions))
  expect_true(all(features_passthrough_and_positions %in% glmnet_results@sb_backtest_workflow[[length(glmnet_results@sb_backtest_workflow)]]$chosen_signals_and_positions))

  adapted_chosen_signals_and_positions <- c("long", "long", unname(features_passthrough_and_positions))
  names(adapted_chosen_signals_and_positions) <-  c(rf_results@backtest_identifier, glmnet_results@backtest_identifier, names(features_passthrough_and_positions))
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@sb_backtest_workflow[[length(sb_metabacktest_results@meta_sb_backtest_results@sb_backtest_workflow)]]$chosen_signals_and_positions,
  ...
  expect_equal(sb_metabacktest_results@mean_validation_metrics$avg_val[c(1:9)], rf_results@consolidated_eval_metrics$avg_val)
  expect_equal(sb_metabacktest_results@mean_validation_metrics$avg_val[c(10:18)], glmnet_results@consolidated_eval_metrics$avg_val)
  expect_equal(unique(sb_metabacktest_results@mean_validation_metrics$sb_backtest),
               c(rf_results@backtest_identifier, glmnet_results@backtest_identifier))
  ...
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[1, -c(1:2)] %>%
                 as.data.frame() %>% tibble::remove_rownames(),
               rf_results@oos_testing_eval_metrics_m_xts@data["2023-04-15/2023-07-15"] %>% as.data.frame() %>% colMeans() %>% t() %>%
                 as.data.frame() %>% tibble::remove_rownames()
  )
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[2, -c(1:2)] %>%
                 as.data.frame() %>% tibble::remove_rownames(),
               glmnet_results@oos_testing_eval_metrics_m_xts@data["2023-04-15/2023-07-15"] %>% as.data.frame() %>% colMeans() %>% t() %>%
                 as.data.frame() %>% tibble::remove_rownames()
  )
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[3, -c(1:2)] %>%
                 as.data.frame() %>% tibble::remove_rownames(),
               meta_results@oos_testing_eval_metrics_m_xts@data %>% as.data.frame() %>% colMeans() %>% t() %>%
                 as.data.frame() %>% tibble::remove_rownames()
  )
  #OOS Eval Metrics
  for (j in 1:length(sb_metabacktest_results@time_series_oos_testing_metrics)){
...
test_that("SW - Metabacktesting works for SW in meta_learning with min_rmse (rf + glmnet with ss are base_sb_results) - features_passthrough = none - meta_ss_config without ss_object", {
  ...
  #Meta-Learner
  expect_message(
  meta_learner_config <- create_sb_backtest_config(sb_algorithm = "sw", training_sample_size = 4, target_fwd_name = "fwd_premium_3m",
                                                   custom_objective = "min_rmse", rebalancing_months = 6, config_name = "meta"),
  ...
  expect_equal(sb_metabacktest_results@base_learners_oos_predictions_m_df@data, oos_predictions_m_df@data)
  #Check if chosen_signals_and_positions flow is correct
  adapted_chosen_signals_and_positions <- c("long", "long")
  names(adapted_chosen_signals_and_positions) <-  c(rf_results@backtest_identifier, glmnet_results@backtest_identifier)
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@sb_backtest_workflow[[length(sb_metabacktest_results@meta_sb_backtest_results@sb_backtest_workflow)]]$chosen_signals_and_positions,
               adapted_chosen_signals_and_positions)
  ...
  #Create custom_signal_universe_m_df
  adapted_custom_signal_universe_metrics_m_df <- derive_adapted_custom_signal_universe_m_df(
    meta_custom_objective = meta_learner_config@custom_objective,
    base_sb_backtest_results_list = base_sb_backtest_results_list,
  ...
  #Check if weights and feat imp matches minimization of rmse
  expected_weights <- (adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-06-15") %>% dplyr::pull(rmse) %>% signal_transform())^(-1)/
    sum((adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-06-15") %>% dplyr::pull(rmse) %>% signal_transform())^(-1))
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@final_sb_model@model@weights, expected_weights)

  ##Feat imp at first rebal
  expected_higher_weight <- adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-04-15") %>% dplyr::slice_min(rmse) %>% dplyr::pull(tickers)
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@feature_importance_m_df@data %>% dplyr::filter(dates == "2023-04-15") %>% dplyr::slice_max(importance) %>% dplyr::pull(tickers),
               expected_higher_weight)

  ##Feat imp at 2nd rebal
  expected_higher_weight <- adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-06-15") %>% dplyr::slice_min(rmse) %>% dplyr::pull(tickers)
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@feature_importance_m_df@data %>% dplyr::filter(dates == "2023-06-15") %>% dplyr::slice_max(importance) %>% dplyr::pull(tickers),
               expected_higher_weight)
  ...
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[1, -c(1:2)] %>% tibble::remove_rownames(),
               rf_results@oos_testing_eval_metrics_m_xts@data["2023-04-15/2023-07-15"] %>% as.data.frame() %>% colMeans()%>% t() %>% as.data.frame()
  )
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[2, -c(1:2)] %>% tibble::remove_rownames(),
               glmnet_results@oos_testing_eval_metrics_m_xts@data["2023-04-15/2023-07-15"] %>% as.data.frame() %>% colMeans()%>% t() %>% as.data.frame()
  )
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[3, -c(1:2)] %>% tibble::remove_rownames(),
...
test_that("SW - Metabacktesting works for SW in meta_learning with max_hr (risk-par + glmnet with ss are base_sb_results) - features_passthrough = none - meta_ss_config without ss_object", {
  ...
  adapted_target_m_df <- create_meta_dataframe(
    toy_preprocessed_targets %>%
      ...
      dplyr::arrange(id)
    , type = "target", meta_dataframe_name = "tg_123")
  ...
  #Meta-Learner
  meta_learner_config <- create_sb_backtest_config(sb_algorithm = "sw", training_sample_size = 4, target_fwd_name = "fwd_premium_3m",
                                                   custom_objective = "max_hr", rebalancing_months = 6, config_name = "meta")

  meta_config <-create_sb_metabacktest_config(meta_sb_backtest_config = meta_learner_config,
                                              base_sb_backtest_results = list(rp_results, glmnet_results),
                                              features_passthrough = c("none"),
                                              config_name = "meta_rf_glmnet")
  ...
  names(base_sb_backtest_results_list) <- c(rp_results@backtest_identifier, glmnet_results@backtest_identifier)
  ...
  expect_true(all(dplyr::pull(rp_results@oos_sb_outputs_m_df@data %>% dplyr::filter(is.na(target)), id) %in%
                    oos_predictions_m_df@data$id))
  #Check if chosen_signals_and_positions flow is correct
  adapted_chosen_signals_and_positions <- c("long", "long")
  names(adapted_chosen_signals_and_positions) <-  c(rp_results@backtest_identifier, glmnet_results@backtest_identifier)
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@sb_backtest_workflow[[length(sb_metabacktest_results@meta_sb_backtest_results@sb_backtest_workflow)]]$chosen_signals_and_positions,
               adapted_chosen_signals_and_positions)


  #Adapted target
  adapted_target_m_df@data <- adapted_target_m_df@data %>% dplyr::filter(id %in% oos_predictions_m_df@data$id)
  ...
  expected_weights <- (adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-06-15") %>% dplyr::pull(hr) %>% signal_transform())/
    sum((adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-06-15") %>% dplyr::pull(hr) %>% signal_transform()))
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@final_sb_model@model@weights, expected_weights)

  ##Feat imp at first rebal
  expected_higher_weight <- adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-04-15") %>% dplyr::slice_max(hr) %>% dplyr::pull(tickers)
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@feature_importance_m_df@data %>% dplyr::filter(dates == "2023-04-15", !tickers == "(Intercept)") %>% dplyr::slice_max(importance) %>% dplyr::pull(tickers),
               expected_higher_weight)

  ##Feat imp at 2nd rebal
  expected_higher_weight <- adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-06-15") %>% dplyr::slice_max(hr) %>% dplyr::pull(tickers)
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@feature_importance_m_df@data %>% dplyr::filter(dates == "2023-06-15", !tickers == "(Intercept)") %>% dplyr::slice_max(importance) %>% dplyr::pull(tickers),
               expected_higher_weight)
  ...
  #Consolidate oos testing metrics
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$cons_oos[c(1:9)], rp_results@consolidated_eval_metrics$cons_oos)
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$cons_oos[c(10:18)], glmnet_results@consolidated_eval_metrics$cons_oos)
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$cons_oos[c(19:27)], meta_results@consolidated_eval_metrics$cons_oos)

  expect_equal(unique(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$testing_dates_range),
               c(paste0(min(rp_results@sb_backtest_workflow[[length(rp_results@sb_backtest_workflow)]]$dates_testing_sample),"-",
                        max(rp_results@sb_backtest_workflow[[length(rp_results@sb_backtest_workflow)]]$dates_testing_sample)),
                 paste0(min(meta_results@sb_backtest_workflow[[length(meta_results@sb_backtest_workflow)]]$dates_testing_sample),"-",
                        max(meta_results@sb_backtest_workflow[[length(meta_results@sb_backtest_workflow)]]$dates_testing_sample))
               ))

  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[1, -c(1:2)] %>% tibble::remove_rownames(),
               rp_results@oos_testing_eval_metrics_m_xts@data["2023-04-15/2023-07-15"] %>% as.data.frame() %>% colMeans()%>% t() %>% as.data.frame()
  )
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[2, -c(1:2)] %>% tibble::remove_rownames(),
               glmnet_results@oos_testing_eval_metrics_m_xts@data["2023-04-15/2023-07-15"] %>% as.data.frame() %>% colMeans()%>% t() %>% as.data.frame()
  )
...
test_that("OLS - Metabacktesting works in integration with run_port_backtest and run_ss_backtest (other base_sb_results (non-validation) + base_ss_results) and sw (max_hr) meta_ss_config without ss_object ", {
  ...
  #SB Configuration and results
  rp_config <- create_sb_backtest_config(sb_algorithm = "rp", training_sample_size = 5, rebalancing_months = 6, target_fwd_name = "fwd_premium_1m",
                                         config_name = "rp_101") %>%
    add_cov_est_method(cov_matrix_sample_size = 2, cov_matrix_benchmark = "ibov")

  ols_config <- create_sb_backtest_config(sb_algorithm = "ols", training_sample_size = 5, rebalancing_months = 6, target_fwd_name = "fwd_premium_1m",
                                          config_name = "glmnet_123")
  ...
  meta_config <- create_sb_metabacktest_config(meta_sb_backtest_config = meta_learner_config,
                                  base_sb_backtest_results = list(rp_results, ols_results),
                                  features_passthrough = c("none"),
  ...
  #Base SB Results
  base_sb_backtest_results_list = list(rp_results, ols_results)
  names(base_sb_backtest_results_list) <- c(rp_results@backtest_identifier, ols_results@backtest_identifier)
  ...
  #Check if chosen_signals_and_positions flow is correct
  adapted_chosen_signals_and_positions <- c("long", "long")
  names(adapted_chosen_signals_and_positions) <-  c(rp_results@backtest_identifier, ols_results@backtest_identifier)
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@sb_backtest_workflow[[length(sb_metabacktest_results@meta_sb_backtest_results@sb_backtest_workflow)]]$chosen_signals_and_positions,
               adapted_chosen_signals_and_positions)


  #Adapted target
  adapted_target_m_df <- create_meta_dataframe(target_m_df)
  adapted_target_m_df@data <- adapted_target_m_df@data %>% dplyr::filter(id %in% oos_predictions_m_df@data$id)
  adapted_target_m_df@meta_dataframe_name <- paste0(adapted_target_m_df@meta_dataframe_name, "_adj")
  ...
  expected_weights <- (adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-03-15") %>% dplyr::pull(hr) %>% signal_transform())/
    sum((adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-03-15") %>% dplyr::pull(hr) %>% signal_transform()))
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@final_sb_model@model@weights, expected_weights)

  ##Feat imp at first rebal
  expected_higher_weight <- adapted_custom_signal_universe_metrics_m_df@data %>% dplyr::filter(dates == "2023-03-15") %>% dplyr::slice_max(hr) %>% dplyr::pull(tickers)
  expect_equal(sb_metabacktest_results@meta_sb_backtest_results@feature_importance_m_df@data %>% dplyr::filter(dates == "2023-03-15", !tickers == "(Intercept)") %>% dplyr::slice_max(importance) %>% dplyr::pull(tickers),
               expected_higher_weight)
  ...
  #Check other outputs (this is a also a test for crete_sb_metabacktest_results)
  #Mean val metrics
  expect_null(nrow(sb_metabacktest_results@mean_validation_metrics))

  #Consolidate oos testing metrics
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$cons_oos[c(1:9)], rp_results@consolidated_eval_metrics$cons_oos)
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$cons_oos[c(10:18)], ols_results@consolidated_eval_metrics$cons_oos)
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$all_dates_oos_testing_metrics$cons_oos[c(19:27)], meta_results@consolidated_eval_metrics$cons_oos)
  ...
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[1, -c(1:2)] %>% tibble::remove_rownames(),
               rp_results@oos_testing_eval_metrics_m_xts@data["2023-03-15/2023-04-15"] %>% as.data.frame() %>% colMeans()%>% t() %>% as.data.frame()
  )
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[2, -c(1:2)] %>% tibble::remove_rownames(),
               ols_results@oos_testing_eval_metrics_m_xts@data["2023-03-15/2023-04-15"] %>% as.data.frame() %>% colMeans()%>% t() %>% as.data.frame()
  )
  expect_equal(sb_metabacktest_results@combined_oos_testing_metrics$common_dates_oos_testing_metrics[3, -c(1:2)] %>% tibble::remove_rownames(),
  ...
  #Val Metrics
  expect_equal(length(sb_metabacktest_results@time_series_validation_metrics), 0)


})

test_that("Skipped: RP - Metabacktesting works in integration with run_port_backtest and run_ss_backtest (other base_sb_results (non-validation) + base_ss_results) and sw (max_hr) meta_ss_config without ", {
  ...
  suppressWarnings(
    ols_results <- run_sb_backtest(
      target_m_df = target_m_df %>% create_meta_dataframe(),
      features_m_df = signals_m_df,
      config = ols_config %>% add_ss_backtest_obj(ss_results),
      parallel = FALSE,
      verbose = TRUE
    )
  )

  #Run Port backtests for sb models
  ols_port_config <- create_port_backtest_config(
    eligibility_quantile_range = c(0.67, 1),
    selected_benchmark = "ibov",
    initial_buffer_period = 5,
    sb_backtest_results = ols_results,
    rebalancing_months = 4,
    port_construction_method = "sw",
    main_liquidity_metric = "mean_volfin_3m",
    config_name = "ols_port"
  ) %>%
    add_liquidity_floor_cutoffs(
      metric_name = c("mean_volfin_3m", "presence"),
      metric_cutoffs = list(
  ...
  rp_port_config <- create_port_backtest_config(
    eligibility_quantile_range = c(0.67, 1),
    selected_benchmark = "ibov",
    initial_buffer_period = 5,
    sb_backtest_results = rp_results,
    rebalancing_months = 4,
    port_construction_method = "sw",
    main_liquidity_metric = "mean_volfin_3m",
    config_name = "rp_port"
  ...
  #Run!
  sb_backtest_config_list <- list(ols_port_config, rp_port_config)
  future::plan("sequential")
  sb_port_backtest_cohort <- purrr::map(sb_backtest_config_list, function(port_config) {
    run_port_backtest(
      signals_m_df = signals_m_df,
      fwd_return_m_df = fwd_return_m_df,
      liquidity_m_df = liquidity_m_df,
  ...
  }) %>% create_port_backtest_cohort(cohort_name = "sb_ports")


  #Mocked Signal Themes
  meta_mocked_signal_themes_m_df <- expand.grid(
    tickers = c("c:ols_port_s:signals_123_f:not_identified", "c:rp_port_s:signals_123_f:not_identified"),
    dates = as.Date(c("2023-03-15", "2023-04-15")),
    stringsAsFactors = FALSE
  ) %>% dplyr::mutate(id = paste0(tickers,"-",dates),
                      theme = dplyr::case_when(
                        tickers %in% c("c:ols_port_s:signals_123_f:not_identified") ~ "ml",
                        tickers %in% c("c:rp_port_s:signals_123_f:not_identified") ~ "heuristic"
                      )
  ) %>%  dplyr::arrange(id) %>% dplyr::select(id, tickers, dates, theme)

  meta_signal_themes_m_df <- create_meta_dataframe(meta_mocked_signal_themes_m_df, "meta_st11", type = "groups")


  #Meta-Learner
  meta_learner_config <- create_sb_backtest_config(sb_algorithm = "rp", training_sample_size = 2, target_fwd_name = "fwd_premium_1m",
                                                   rebalancing_months = 6, config_name = "meta")
  ...
  suppressWarnings(
    sb_metabacktest_results <- run_sb_backtest(
      ...
      features_m_df = signals_m_df,
      config = meta_config,
      base_benchmark_returns_m_xts = benchmark_returns_m_xts,
      base_port_backtest_cohort = port_backtest_cohort,
      meta_port_backtest_cohort = sb_port_backtest_cohort,
      parallel = FALSE,
      verbose = TRUE
    )
  )
...
test_that("run_sb_backtest correctly classifies data as training, validation and testing", {
   ...
   load(paste(test_path(),"/testdata/","toy_fulldates_features_and_targets.RData", sep =""))
    ...
    xgb_config <- create_sb_backtest_config(sb_algorithm = "xgb", custom_objective = "pseudo_huber_error",
                                            target_fwd_name = c("fwd_premium_3m"), training_sample_size = 60, rebalancing_months = 6) %>%
  ...
  rebalance_dates <- as.Date(zoo::index(sb_backtest_results@validation_eval_metrics_hyper_choice_m_xts@data), format = "%Y-%m-%d")
...
test_that("run_sb_backtest does not works with NAs in last target_fwd+ 1 periods of target_m_df",{
  ...
  glmnet_config <- create_sb_backtest_config(sb_algorithm = "glmnet", training_sample_size = 7, rebalancing_months = 6,target_fwd_name = c("fwd_premium_3m")) %>%
    add_tuning_strategy(tuning_method = "random_search", validation_sample_size = 3, chosen_eval_metric = "rss", n_iter = 5) %>%
    add_hyperparameter(hyperparameter = c("alpha", "lambda.min.ratio"),
                       distribution_choice = c("uniform", "uniform"),
                       pars = list(c(min = 0, max = 1), c(min = 0.1, max = 0.9))
