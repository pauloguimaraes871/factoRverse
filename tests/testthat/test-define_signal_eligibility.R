test_that("define_signal_elibility works for no-pooled setting", {
  ...
  expected_result <- summarize_performance(
    ...
    selected_signal_themes_m_d_ref = selected_signal_themes_m_d_ref, selected_backtest_returns_corrected_positions_xts_upd_ref = selected_backtest_returns_corrected_positions_xts_upd_ref,
  ...
  expect_equal(results$frequentist_results$Alpha$coefficients, expected_result$frequentist_fit_results_list$Alpha$coefficients)
  expect_equal(results$frequentist_results$Gamma$coefficients, expected_result$frequentist_fit_results_list$Gamma$coefficients)
  expect_equal(results$frequentist_results$low_Beta$coefficients, expected_result$frequentist_fit_results_list$low_Beta$coefficients)
...
test_that("define_signal_elibility works for pooled setting", {
  ...
  expected_result$signal_universe_m_d_ref <- classify_investment_universe(expected_result$signal_universe_m_d_ref, signal_significance_threshold = 0.15,
                                                           groups_m_d_ref = selected_signal_themes_m_d_ref,
                                                           concentration_constraint_policy = concentration_constraint_policy_test,
                                                           asset_object = "signals")
  ...
  expect_equal(results$signal_universe_m_d_ref,expected_result$signal_universe_m_d_ref)
  expect_equal(coef(results$frequentist_results), coef(expected_result$frequentist_fit_results_list))
...
test_that("define_signal_elibility works for no-pooled frequentist setting when there is a backtest with short length", {
  ...
  #expected results
  expected_result <- suppressWarnings(summarize_performance(selected_backtest_returns_corrected_positions_xts_upd_ref = selected_backtest_returns_corrected_positions_xts_upd_ref,
                                            selected_market_factor_proxy_xts_upd_ref = selected_market_factor_proxy_xts_upd_ref,
                                            selected_signal_themes_m_d_ref = selected_signal_themes_m_d_ref,
                                            model_structure = "no_pooled", model_spec_theme_level = NULL,
                                            lmer_control =  list(lmer_optimizer = "nloptwrap", lmer_optimization_objective = "REML", hierarchical_p_value_method = "Satterthwaite"),
