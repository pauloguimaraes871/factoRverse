test_that("summarize_performance works for no_pooled model_structure", {
  ...
  model_list <- list()
  for(j in 1:3){
    model_list[[j]] <- lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[,j] ~ selected_market_factor_proxy_m_xts_upd_ref)
    alpha[j] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[,j] ~ selected_market_factor_proxy_m_xts_upd_ref)))[1,1]
    alpha_se[j] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[,j] ~ selected_market_factor_proxy_m_xts_upd_ref)))[1,2]
    beta[j] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[,j] ~ selected_market_factor_proxy_m_xts_upd_ref)))[2,1]
    specific_risk[j] <- sigma(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[,j] ~ selected_market_factor_proxy_m_xts_upd_ref))
    alpha_t_stat[j] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[,j] ~ selected_market_factor_proxy_m_xts_upd_ref)))[1,3]
    p_value[j] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[,j] ~ selected_market_factor_proxy_m_xts_upd_ref)))[1,4]/2
  }
  treynor_ratio <- apply(selected_backtest_returns_corrected_positions_m_xts_upd_ref, 2, function(x){
   PerformanceAnalytics::mean.geometric(x/100)*100
  })/beta
  app_ratio <- alpha/specific_risk

  #Result
  result <- summarize_performance(selected_backtest_returns_corrected_positions_m_xts_upd_ref = selected_backtest_returns_corrected_positions_m_xts_upd_ref,
                                  selected_market_factor_proxy_m_xts_upd_ref = selected_market_factor_proxy_m_xts_upd_ref,
...
test_that("summarize_performance works for no_pooled model_structure and NAs", {
  ...
  selected_backtest_returns_corrected_positions_m_xts_upd_ref$Alpha[1:2] <- NA
  #Create base_signal_universe_m_d_ref
  base_signal_universe_m_d_ref <- create_performance_m_df(
    selected_backtest_returns_corrected_positions_m_xts_upd_ref = selected_backtest_returns_corrected_positions_m_xts_upd_ref,
    selected_market_factor_proxy_m_xts_upd_ref = selected_market_factor_proxy_m_xts_upd_ref,
  ...
  alpha[1] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[3:4,1] ~ selected_market_factor_proxy_m_xts_upd_ref[3:4])))[1,1]
  alpha_se[1] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[3:4,1] ~ selected_market_factor_proxy_m_xts_upd_ref[3:4])))[1,2]
  beta[1] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[3:4,1] ~ selected_market_factor_proxy_m_xts_upd_ref[3:4])))[2,1]
  specific_risk[1] <- sigma(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[3:4,1] ~ selected_market_factor_proxy_m_xts_upd_ref[3:4]))
  alpha_t_stat[1] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[3:4,1] ~ selected_market_factor_proxy_m_xts_upd_ref[3:4])))[1,3]
  model_list[[1]] <- lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[3:4,1] ~ selected_market_factor_proxy_m_xts_upd_ref[3:4])
  for(j in 2:3){
    alpha[j] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[,j] ~ selected_market_factor_proxy_m_xts_upd_ref)))[1,1]
    alpha_se[j] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[,j] ~ selected_market_factor_proxy_m_xts_upd_ref)))[1,2]
    beta[j] <- coef(summary(lm(selected_backtest_returns_corrected_positions_m_xts_upd_ref[,j] ~ selected_market_factor_proxy_m_xts_upd_ref)))[2,1]
...
test_that("summarize_performance works for pooled model_structure and NAs", {
  ...
  selected_signals_corrected_positions_m_df <- selected_signals_and_backtest_list$selected_signals_corrected_positions_m_df
  selected_backtest_returns_corrected_positions_m_xts <- selected_signals_and_backtest_list$selected_backtest_returns_corrected_positions_m_xts
  selected_backtest_returns_corrected_positions_m_xts$Gamma[c(1:4)] <- NA
  selected_backtest_returns_corrected_positions_m_xts$Alpha[c(1:2)] <- NA
  ...
  result <- suppressWarnings(summarize_performance(selected_backtest_returns_corrected_positions_m_xts_upd_ref = selected_backtest_returns_corrected_positions_m_xts_upd_ref,
                                  selected_market_factor_proxy_m_xts_upd_ref = selected_market_factor_proxy_m_xts_upd_ref,
                                  model_structure = "partial_pooled", model_spec_theme_level = "theme_specific_intercept_fixed_slope",
                                  lmer_control = list(lmer_optimizer = "Nelder_Mead", lmer_optimization_objective  = "REML", hierarchical_p_value_method = "Satterthwaite"),
                                  selected_signal_themes_m_d_ref = selected_signal_themes_m_d_ref
