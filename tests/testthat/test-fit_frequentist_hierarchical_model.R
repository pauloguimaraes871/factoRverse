test_that("fit_frequentist_hierarchical_model works for random_intercept_fixed_slope", {
  ...
  selected_backtest_returns_corrected_positions_m_upd_ref <- data.frame(
    ...
    return = c(selected_backtest_returns_corrected_positions_m_xts_upd_ref$Alpha, selected_backtest_returns_corrected_positions_m_xts_upd_ref$Gamma,
               selected_backtest_returns_corrected_positions_m_xts_upd_ref$low_Beta),
    market_factor_proxy = rep(selected_market_factor_proxy_m_xts_upd_ref, each = 3),
  ...
  pooled_capm_m_d_ref$treynor_ratio <-
    as.numeric(PerformanceAnalytics::mean.geometric(selected_backtest_returns_corrected_positions_m_xts_upd_ref/100))*100/pooled_capm_m_d_ref$individual_beta
  ...
  expect_equal(tibble::tibble(result$pooled_CAPM_metrics_m_d_ref),
               tibble::tibble(pooled_capm_m_d_ref) %>% dplyr::arrange(id)
               )
...
test_that("fit_frequentist_hierarchical_model works for fixed_intercept_fixed_slope", {
  ...
  #wrong opt
  expect_error(fit_frequentist_hierarchical_model(
    signal_universe_m_d_ref = base_signal_universe_m_d_ref,
    selected_backtest_returns_corrected_positions_m_xts_upd_ref = selected_backtest_returns_corrected_positions_m_xts_upd_ref,
    selected_market_factor_proxy_m_xts_upd_ref = selected_market_factor_proxy_m_xts_upd_ref,
    selected_signal_themes_m_d_ref = selected_signal_themes_m_d_ref,
    model_spec_theme_level = "fixed_intercept_fixed_slope",
    lmer_optimizer = "Nelder", lmer_optimization_objective = TRUE, hierarchical_p_value_method = "Kenward-Roger"
  ), "Invalid optimizer. Please choose from 'nloptwrap', 'bobyqa', 'Nelder_Mead' or 'nlminbwrap'.")
  ...
  expect_error(fit_frequentist_hierarchical_model(
    ...
    model_spec_theme_level = "fixed_intercept_fixed_slope",
    lmer_optimizer = "Nelder_Mead", lmer_optimization_objective = "TRUE", hierarchical_p_value_method = "Kenward-Roger"
  ), "Invalid optimization objective. Please choose from TRUE or FALSE.")
