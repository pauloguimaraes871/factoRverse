...
test_that("estimate_covariance_matrix works for shrink_id and active returns", {
  ...
  expected_results <- PerformanceAnalytics::M2.shrink(as.matrix(expected_results), target = 2)
  expected_results <- expected_results$M2sh
  ...
  results <- estimate_covariance_matrix(
    ...
    returns_m_xts_upd_ref = daily_stock_returns_m_xts,
    cov_matrix_sample_size = covariance_matrix_sample_size,
    cov_estimation_method = "shrink_id",
    active_returns = TRUE,
    selected_benchmark_m_xts_upd_ref = selected_benchmark_returns_m_xts_upd_ref,
    groups_m_d_ref = stocks_groups_m_d_ref
  )
...
test_that("estimate_covariance_matrix works for not providing sample size", {
  #Load
  load(paste(test_path(),"/testdata/","artificial_port_obj.RData", sep =""))
...
test_that("estimate_covariance_matrix throws errors for wrong tickers and methods", {
  ...
  expect_error(
    estimate_covariance_matrix(
      tickers = c("Stock A", "Stock B", "Stock C", "Stock F"),
      returns_m_xts_upd_ref = daily_stock_returns_m_xts,
      cov_matrix_sample_size = covariance_matrix_sample_size,
      active_returns = TRUE,
      selected_benchmark_m_xts_upd_ref = selected_benchmark_returns_m_xts_upd_ref
    ), "Tickers without correspondence in returns_m_xts_upd_ref"
  )
  ...
  expect_error(
    estimate_covariance_matrix(
      ...
      active_returns = TRUE,
      selected_benchmark_m_xts_upd_ref = selected_benchmark_returns_m_xts_upd_ref
    ), "cov_estimation_method not supported"
  )
