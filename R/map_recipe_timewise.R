...
# step_winsorize------------------------------------
#' @title Step for Winsorization
#'
#' @description
#' `step_winsorize()` creates a *recipe step* that applies winsorization to numeric variables.
#' Winsorization replaces extreme values below or above specified quantile thresholds with those threshold values.
#'
#' This step is useful to reduce the influence of outliers without fully trimming observations.
#'
#' @param recipe A `recipe` object. The step will be added to the sequence of operations for this recipe.
#' @param ... One or more selector functions to choose which variables to apply winsorization to. If empty, all numeric variables are selected.
#' @param role Not used by this step since no new variables are created. Defaults to `NA`.
#' @param trained A logical indicating whether the step has been trained (i.e., thresholds have been calculated).
#' @param probs A numeric vector of length 2 indicating the lower and upper quantile probabilities used to determine winsorization limits. Default is `c(0.05, 0.95)`.
#' @param skip A logical. Should the step be skipped when the recipe is baked by `recipes::bake()`? Default is `FALSE`.
#' @param id A character string used to identify this step. Defaults to a random string generated by `recipes::rand_id("winsorize")`.
#'
#' @return An updated `recipe` object with the `step_winsorize` added to the sequence of steps.
#'
#' @details
...
#'
#' This is a low-level constructor for the `step_winsorize` preprocessing step. It is called internally
#' by `step_winsorize()` and is not intended to be used directly by users.
#'
#' @param terms A list of quosures generated by `rlang::enquos()` specifying which columns to transform.
#' @param role Not used by this step as no new variables are created. Included for consistency with the `recipes` API.
#' @param trained A logical indicating whether the step has been trained. When `TRUE`, `winsor_limits` must be provided.
#' @param probs A numeric vector of length 2 indicating the lower and upper quantile probabilities used for winsorization.
#' @param winsor_limits A named list of length-two numeric vectors (per variable) with the computed quantile cutoffs (added during `prep()`).
#' @param skip A logical. Should the step be skipped when the recipe is baked by `recipes::bake()`? Defaults to `FALSE`.
#' @param id A character string that uniquely identifies the step.
#'
...
#' @param ... Additional arguments passed to methods (not used).
#' @param sector A character string specifying the name of the sector variable.
#' @param method A character string specifying the imputation method. Either "mean" or "median".
#' @param role The role to assign to the new variables created by this step. Default is `NA`.
#' @param trained A logical indicating if the step has been trained. Default is `FALSE`.
...
#'
#' Low-level constructor function for creating a new `step_impute_sector` step in a recipe.
#' This step is designed to impute missing values by group (e.g., sectors), using group-specific means or medians.
#' The constructor is typically used internally and should not be called directly by users.
#'
#' @param terms A `quosure` with the columns to be imputed, as selected via `recipes::terms_select()`.
#' @param sector A `character` string specifying the name of the column in the data used to group observations (e.g., "sector").
#' @param method A `character` string specifying the imputation method to use within each group. Typically "mean" or "median".
#' @param role A character string describing the role of the created variable (usually `"predictor"`). This is required by the `recipes` API.
#' @param trained A logical indicating whether the preprocessing step has been trained.
#' @param impute_values A list of named vectors storing the imputed values computed during training (one vector per group).
#' @param skip A logical. Should the step be skipped when the recipe is baked by `recipes::bake()`? Default is `FALSE`.
#' @param id A character string used to uniquely identify the step. Automatically generated if `NULL`.
#'
...
#' @title Tidy Method for step_impute_sector
#'
#' @description
#' Generates a tidy summary of the `step_impute_sector` object, showing which columns were imputed,
#' the method used, and the imputed values (if trained).
#'
#' @param x A `step_impute_sector` object.
#' @param ... Not used. Included for method compatibility.
#'
#' @return A tibble with columns:
#' \itemize{
#'   \item `column`: The names of the variables imputed.
#'   \item `sector`: The sector used for group-wise imputation (only if not trained).
#'   \item `method`: Imputation method ("mean", "median", etc.).
#'   \item `imputed_value`: Imputed values for each sector-variable pair (if trained).
