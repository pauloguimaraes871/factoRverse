...
#' @param returns_xts_upd_ref A dataframe in which columns represent tickers present in current_universe_df and row represent days
...
#' returns_xts_upd_ref will be adjusted by subtracting the selected market factor proxy in benchmark_returns_xts.
#'
#' @return
#' @export
#'
#' @examples
estimate_covariance_matrix <- function(tickers, returns_xts_upd_ref,
  ...
  #Checks
  if(!all(tickers %in% colnames(returns_xts_upd_ref))){
    stop("Tickers without correspondence in returns_xts_upd_ref")
  }
  ...
  if(is.null(cov_matrix_sample_size)){
    ...
    dates_to_sample <- returns_xts_upd_ref_dates[(n_dates - cov_matrix_sample_size):n_dates]
  }

  #Get all rows that comprehend current_date - cov_matrix_sample_size
  returns_xts_sample <- returns_xts_upd_ref[which(returns_xts_upd_ref_dates %in% dates_to_sample), #Get all dates in dates_to_sample
  ...
  if(active_returns){
    ...
    selected_benchmark_xts_sample <- selected_benchmark_xts_upd_ref[which(zoo::index(selected_benchmark_xts_upd_ref) %in% dates_to_sample),]

    #Get decimals
    returns_xts_sample_decimals <- returns_xts_sample/100
    selected_benchmark_xts_sample_decimals <- as.vector(selected_benchmark_xts_sample/100)

    ##Get geometric active returns
    ###returns_xts_sample_decimals
    returns_xts_sample_decimals <- xts::xts(
      sapply(
        #For each series
        colnames(returns_xts_sample_decimals), function(series) {
          #Apply geometric return difference formula
          purrr::map2_dbl(
            returns_xts_sample_decimals[, series], #.x
            selected_benchmark_xts_sample_decimals, #.y
            ~ (1 + .x) / (1 + .y) - 1 #.f
          )
        }
      ),
      order.by = zoo::index(returns_xts_sample_decimals)
    )

    #Turn back to percentages
    returns_xts_sample <- returns_xts_sample_decimals*100
    selected_benchmark_xts_sample <- selected_benchmark_xts_sample_decimals*100
  ...
  ###############################
  returns_xts_sample_clean <- clean_returns_sample(returns_xts_sample = returns_xts_sample, #Returns
