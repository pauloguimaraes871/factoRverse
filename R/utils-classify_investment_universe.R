...
#'
#' @param signals_m_d_ref A data frame of stocks with signals columns, as generated by the `generate_signal_matrix` function.
...
classify_investment_universe <- function(signals_m_d_ref, #Signals d_ref
  ...
  if(asset_object == "signals"){
    ...
    #Create SB Benchmark
    benchmark_weights_m_d_ref <- create_sb_benchmark(signal_universe_m_d_ref = universe_m_d_ref, signals_groups_m_d_ref = groups_m_d_ref)
  ...
  if(!is.null(concentration_constraint_policy$benchmark)){ #If max_abs_active_individual_weight is NULL, do not apply rule
    ...
    if(!is.null(concentration_constraint_policy$max_abs_active_individual_weight)){
      max_abs_active_weight_individual_rule_m_d_ref$max_abs_aw_ind <- #Apply Maximum Absolute Active Individual Weight Rule
        ifelse(max_abs_active_weight_individual_rule_m_d_ref[,2] >=
                 concentration_constraint_policy$max_abs_active_individual_weight, 1L, 0L) #Is stock relevant in benchmark?
      }
