...
sur_rolling <- function(main_matrix, complementary_matrix){
  ...
  if (!is.matrix(main_matrix) && !is.data.frame(main_matrix) && !is_tibble(main_matrix)) {
    stop("main_matrix must be a matrix, data frame, or tibble.")
  }

  if (!is.matrix(complementary_matrix) && !is.data.frame(complementary_matrix) && !is_tibble(complementary_matrix)) {
  ...
  # Initialize the SUR matrix
  sur_matrix <- matrix(NA, nrow = nrow(main_matrix), ncol = ncol(main_matrix))
