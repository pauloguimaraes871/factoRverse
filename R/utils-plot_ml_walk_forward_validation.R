...
#'
#' This function generates various plots to visualize the performance of machine learning models using walk-forward validation metrics. It creates plots comparing out-of-sample (OOS) testing metrics, validation metrics, and hyperparameter performance over time.
#'
#' @param oos_testing_eval_metrics A data frame containing out-of-sample testing evaluation metrics. The data frame should have dates as row names and metric values as columns. The dates should be in "YYYY-MM-DD" format.
#' @param validation_eval_metrics_hyper_choice A data frame containing validation evaluation metrics and hyperparameter choices. The data frame should have dates as row names and metric values as columns. The dates should be in "YYYY-MM-DD" format.
#' @param hyper_choice_df A data frame containing hyperparameter choices and their corresponding evaluation metrics. The data frame should have dates as row names and hyperparameter values as columns.
#' @param chosen_eval_metric A string specifying the chosen evaluation metric for analysis. This metric must be present in both the OOS testing and validation data frames.
#' @param chosen_eval_metric_validation A list of data frames, where each data frame contains hyperparameter choices and evaluation metrics for validation. The list should have one data frame for each validation period.
#' @param ml_algorithm A string specifying the machine learning algorithm used. It determines how hyperparameter performance is plotted. Possible values include "glmnet", "rf", "xgb", and "nn".
#' @param show_plots A logical value indicating whether to display the plots. Default is `TRUE`.
#' @param rebalance_dates A vector of dates indicating the times when model rebalancing occurred. These dates are used to add vertical lines to some plots.
#'
