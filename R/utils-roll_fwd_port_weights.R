...
roll_fwd_port_weights <- function(port_weights_m_d_ref, clean_fwd_return_1m_m_d_ref){
  ...
  rolled_fwd_port_weights_m_d_ref <- rolled_fwd_port_weights_m_d_ref %>% dplyr::left_join(clean_fwd_return_1m_m_d_ref %>%
                                                                                            dplyr::select(id, fwd_return_1m),
                                                                                          by = "id")
  ...
  rolled_fwd_port_weights_m_d_ref <- rolled_fwd_port_weights_m_d_ref %>%
    ...
    dplyr::select(-dplyr::any_of(c("eop_port_weights", "fwd_return_1m", "bench_weights"))) #Remove eop_port_weights, fwd_return_1m and possibly bench-weights
