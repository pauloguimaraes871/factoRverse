...
| 1 | Characteristic portfolios (`run_port_backtest()`) | 9 weighting methods (EW to MVO and MMAF); liquidity, turnover and concentration rules; transaction costs and trade log | PortfolioAnalytics, riskParityPortfolio, PerformanceAnalytics |
| 2 | Signal selection (`run_ss_backtest()`) | FWER/FDR control; no-pooled or partial-pooled hierarchical CAPM; Bayesian shrinkage with user or data-informed priors | lme4, lmerTest, brms/Stan, tidybayes |
| 3 | Signal blending (`run_sb_backtest()`) | ML and heuristic blenders; grid, random or Bayesian tuning; feature importance; meta-ensembles | glmnet, ranger, xgboost, keras/TensorFlow, ParBayesianOptimization |
| 4 | Deployment (`run_port_backtest()` with `sb_results`) | Blended score drives the final book under full constraints, costs and audit trail | PortfolioAnalytics, riskParityPortfolio |

Under the hood, three modular engines are deliberately reused across
workflows: `set_portfolio_weights()` builds every portfolio in workflows
1 and 4 and powers the heuristic blenders of workflow 3 (enabling factor
timing); `classify_investment_universe()` applies one eligibility
grammar to stocks (workflow 1) and to signals (workflow 2); and
`summarize_performance()` computes CAPM metrics both for target creation
(stage 0) and for signal selection (workflow 2).
...
Every `*_results` object also answers `plot()` with an interactive menu:
76 built-in plots across the five results classes (18 for portfolio
backtests, 16 for cohorts, up to 16 for signal selection including
posterior diagnostics, up to 18 for signal blending including the
feature-importance suite, and 8 for meta-ensembles), all in a consistent
palette.

A richer visual version of this map, with per-workflow features and the
full plot inventory, lives on the package website:
[FactorOps map](https://pauloguimaraes871.github.io/factoRverse/factorops-map.html).
...
- **Neural-network signal blending** needs Python with TensorFlow and
  ...
  installing `factoRverse`, run `keras::install_keras()` once to set up
...
individual help pages (e.g. `?run_port_backtest`) for argument-level
detail.
...
listing/delisting bookkeeping) and `create_target_m_df()` (dozens of
forward, risk-adjusted targets) round out the data layer.
...
`create_sb_metabacktest_config()`, and predictions are interpretable end
to end via global-surrogate feature importance and
`explain_prediction()`.
